...
## 更新摘要
**变更内容**
- 新增间隔选择选项 `--interval`，支持 daily、1min、5min、15min、30min、60min
- 调整默认初始资本设置为300000.0
- 新增 `--initial-capital` 别名参数，与 `--initial-cash` 共存
- 新增 `--report` 参数，允许用户指定Markdown报告输出文件路径
- 集成新的markdown报告生成功能，支持中文报告格式
- 更新Docker脚本中的CLI参数映射
...
**更新** 本次更新反映了CLI功能增强，新增间隔选择选项、默认初始资本设置调整以及新增的报告生成功能，提升了用户在不同时间框架下的分析能力和报告输出能力。
...
REPORT["src/utils/display.py<br/>Markdown报告生成"]
end
subgraph "回测子系统"
...
## 核心组件
- 通用CLI输入解析器：负责股票代码、日期范围、分析师选择、模型选择、资金参数、**新增间隔选择**、**新增报告参数**等的解析与交互式选择。
- 主程序CLI入口：整合输入解析、工作流构建、执行与输出展示，**新增报告生成与保存功能**。
- 回测CLI与引擎：支持批量化历史回测、性能指标输出与进度展示。
- 输出与可视化：交易决策表格、组合汇总、进度条、图谱导出PNG。
- 模型与分析师配置：统一管理可用模型列表、提供商、分析师节点映射。
- **新增报告系统**：支持生成中文Markdown格式的投资分析报告，包含决策建议、信号分布、分析师分析等内容。

**更新** 新增的报告生成功能允许用户将分析结果保存为结构化的Markdown文件，便于分享和归档。
...
## 架构总览
CLI整体采用"输入解析 → 参数校验 → 工作流编译 → 执行 → 输出"的流水线设计。主程序与回测器共享同一套输入解析逻辑，但执行路径不同：主程序实时交易决策，回测器按日回放历史数据并计算收益曲线与风险指标。**新增的报告系统在执行完成后自动生成并保存Markdown格式的分析报告**。
...
CLI->>Report : 生成并保存Markdown报告
Report-->>User : 显示报告保存路径
...
- 功能要点
  ...
  - 资金参数：**新增别名参数** `--initial-capital` 与 `--initial-cash`，默认值调整为300000.0。
  - **新增间隔选择**：支持 `daily`、`1min`、`5min`、`15min`、`30min`、`60min` 等数据间隔。
  - **新增报告参数**：支持 `--report` 指定Markdown报告输出文件路径。
  - 可选开关：显示推理过程、显示代理图。
- 参数校验与错误提示
  - 日期格式严格校验，不符合则抛出异常。
  - 未选择分析师时中断退出并提示。
...
ResolveDates --> SelectInterval["选择数据间隔"]
SelectInterval --> SelectReport["选择报告输出路径"]
SelectReport --> ValidateDates{"日期格式有效？"}
...
- 入口行为
  ...
  - **新增报告生成**：如果设置了`--report`参数，则调用报告生成函数生成Markdown报告并保存到指定文件。
- 关键流程
  - 进度跟踪：开始/停止由进度模块管理。
  - 图谱导出：可选保存为PNG。
  - 输出展示：使用表格化方式呈现每个标的的信号、决策与组合汇总。
  - **新增间隔传递**：将解析的间隔参数传递给执行引擎。
  - **新增报告保存**：调用`generate_markdown_report`函数生成报告内容并写入文件。
...
- 回测CLI
  - 支持股票代码、日期范围、**调整后的初始资金**、保证金比例、分析师选择、Ollama开关。
...
- 输出模块
  ...
  - **新增报告生成**：`generate_markdown_report`函数支持生成中文Markdown格式的分析报告。
- 进度模块
...
### 报告系统（src/utils/display.py）
- **新增功能**：`generate_markdown_report`函数
  - 支持生成中文Markdown格式的投资分析报告
  - 包含股票代码、分析时间、模型信息、分析师数量、初始本金等基本信息
  - 生成最终交易决策、信号分布、各分析师详细分析等章节
  - 支持时间段操作建议（短期、中期、长期）
  - 自动翻译分析师理由为中文
  - 生成标准的Markdown格式，便于分享和归档

**章节来源**
- [src/utils/display.py:691-890](file://src/utils/display.py#L691-L890)

## 依赖分析
- 输入解析器依赖
  - 与模型配置（LLM/OLLAMA）、分析师配置、Ollama可用性检查集成。
- 主程序依赖
  - 与输入解析器、输出模块、进度模块、可视化模块、**报告系统**耦合，形成闭环。
- 回测CLI/引擎依赖
...
- **新增间隔优化**：不同时间间隔的数据获取策略，平衡回测精度与性能。
- **新增报告性能**：报告生成采用批量翻译策略，减少LLM调用次数，提高效率。
...
- **新增间隔问题**
  - 现象：选择的间隔不适用于某些数据源或市场。
  - 处理：尝试更长的时间间隔，如从1min切换到5min或daily。
- **新增报告问题**
  - 现象：报告生成失败或无法保存。
  - 处理：检查文件路径权限；确认有足够的磁盘空间；验证模型配置正确。
...
## 结论
本CLI以统一的输入解析器为核心，分别驱动主程序的实时交易决策与回测器的历史回放。通过交互式选择、参数校验与错误提示、进度显示与可视化导出，既满足新手快速上手，也支持高级用户的批量与自动化集成。**新增的报告生成功能进一步增强了CLI的价值，用户可以轻松生成结构化的分析报告用于分享和归档**。配合Docker与Windows脚本，可在多种环境下高效运行。

**更新** 新增的间隔选择功能、默认初始资本设置调整以及报告生成功能进一步增强了CLI的灵活性和实用性，为不同用户需求提供了更好的支持。
...
- 主程序（实时交易）
  - 必需：股票代码（--tickers）
  - 可选：--start-date/--end-date、**调整后的初始资金**（--initial-cash/--initial-capital，默认300000.0）、--margin-requirement、--analysts、--analysts-all、--ollama、--model、--show-reasoning、--show-agent-graph、**新增间隔**（--interval）、**新增报告**（--report）
- 回测器（历史回放）
  - 可选：--tickers、--start-date/--end-date、**调整后的初始资金**（--initial-capital，默认300000）、--margin-requirement、--analysts、--analysts-all、--ollama、**新增间隔**（--interval）
- Docker运行
  - run.bat支持：--ticker、--start-date、--end-date、**调整后的初始资金**（--initial-cash，默认300000.0）、--margin-requirement、--ollama、--ollama-base-url、--show-reasoning、main/backtest/build/compose/ollama/pull/help
- Windows应用脚本
  - app\run.bat用于一键启动Web应用，包含依赖检查与服务启动流程。

**更新** 参数说明已更新以反映新的默认值、新增的间隔选择功能以及新增的报告参数。
...
- 支持的时间间隔：
  - `daily`：日线数据，适合长期趋势分析
  - `1min`：1分钟数据，适合高频交易策略验证
  - `5min`：5分钟数据，平衡频率与性能
  - `15min`：15分钟数据，适合中频交易策略
  - `30min`：30分钟数据，适合较长时间框架
  - `60min`：60分钟数据，适合日线级别分析
- 默认值：`daily`
- 适用场景：根据策略类型和数据可用性选择合适的间隔

#### 默认初始资本设置
- 默认值：300000.0（美元）
- 参数别名：
  - `--initial-cash`：传统参数名
  - `--initial-capital`：新别名参数
- 用途：设置初始投资资金，影响交易规模和风险控制

#### 报告生成功能（--report）
- 功能描述：生成并保存Markdown格式的投资分析报告
- 参数语法：`--report <文件路径>`
- 输出格式：中文Markdown文件，包含股票分析、决策建议、信号分布、分析师分析等内容
- 文件命名：自动包含股票代码、日期和时间戳
- 适用场景：需要生成结构化分析报告用于分享、归档或审计

**章节来源**
- [src/cli/input.py:64-70](file://src/cli/input.py#L64-L70)
- [src/cli/input.py:250-257](file://src/cli/input.py#L250-L257)
- [src/backtesting/cli.py:33](file://src/backtesting/cli.py#L33)
- [docker/run.bat:27](file://docker/run.bat#L27)
- [src/main.py:184-198](file://src/main.py#L184-L198)
- [src/utils/display.py:691-890](file://src/utils/display.py#L691-L890)