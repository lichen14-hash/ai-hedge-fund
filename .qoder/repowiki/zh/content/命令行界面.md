...
**更新** 本次更新反映了CLI功能增强，新增间隔选择选项和默认初始资本设置调整，提升了用户在不同时间框架下的分析能力。
...
SelectInterval --> ValidateDates{"日期格式有效？"}
...
**更新** 新增的间隔选择功能和默认初始资本设置调整进一步增强了CLI的灵活性和实用性，为不同用户需求提供了更好的支持。
...
- 主程序（实时交易）
  - 必需：股票代码（--tickers）
  - 可选：--start-date/--end-date、**调整后的初始资金**（--initial-cash/--initial-capital，默认300000.0）、--margin-requirement、--analysts、--analysts-all、--ollama、--model、--show-reasoning、--show-agent-graph、**新增间隔**（--interval）
- 回测器（历史回放）
