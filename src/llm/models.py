...
def get_model(model_name: str, model_provider: ModelProvider, api_keys: dict = None) -> ChatOpenAI | ChatGroq | ChatOllama | GigaChat | None:
    if model_provider == ModelProvider.GROQ:
        ...
        return ChatXAI(model=model_name, api_key=api_key, temperature=0)
    elif model_provider == ModelProvider.GIGACHAT:
        if os.getenv("GIGACHAT_USER") or os.getenv("GIGACHAT_PASSWORD"):
            return GigaChat(model=model_name, temperature=0)
        else: 
            ...
            return GigaChat(credentials=api_key, model=model_name, temperature=0)
        ...
        return AzureChatOpenAI(azure_endpoint=azure_endpoint, azure_deployment=azure_deployment_name, api_key=api_key, api_version="2024-10-21", temperature=0)
